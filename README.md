...
- 十一款个人邮件主题、站内通知和邮件失败重试；双方连续完成约定会逐步解锁线条小狗、噜噜、奶龙、一二布布和猫鼠追光主题。列表支持服务端分页、约定搜索及筛选，旧邮件链接可以直接定位历史记录。
...
「我们 → 邮件提醒」可选择十一款外观。基础六款立即可用；双方连续每天各完成一件约定，达到 2、7、14、21、30 天后，依次可以领取新的可爱主题。预览不发邮件，也可打开 [独立邮件预览册](docs/email-preview.html)。邮件由 worker 异步发送，最多自动尝试 5 次，失败可重试。SMTP 故障窗口可能产生重复邮件，积分和订单仍由事务与幂等约束防重。
