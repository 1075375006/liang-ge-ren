...
ENV_FILE=${DEPLOY_ENV_FILE:-"$ROOT/production.env"}
case "$ENV_FILE" in /*) ;; *) echo 'DEPLOY_ENV_FILE must be an absolute path' >&2; exit 1;; esac
...
clear_compose_overrides() {
  ...
  for key in COMPOSE_PROJECT_NAME COMPOSE_ENV_FILES COMPOSE_PROFILES APP_IMAGE APP_VERSION POSTGRES_IMAGE POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DB APP_URL BIND_ADDRESS APP_PORT COOKIE_SECURE TRUST_PROXY LOG_LEVEL REGISTRATION_OPEN REQUIRE_VERIFIED_EMAIL SUPPORT_EMAIL OPERATOR_NAME DEPLOY_STATE_DIR WECHAT_LOGIN_ENABLED BEICHEN_APP_ID BEICHEN_APP_KEY SMTP_HOST SMTP_PORT SMTP_SECURE SMTP_REQUIRE_TLS SMTP_USER SMTP_PASS SMTP_FROM BACKUP_DIR BACKUP_UID BACKUP_GID BACKUP_RETENTION_DAYS; do
    unset "$key"
  done
}
compose() (
