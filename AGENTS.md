...
- 生产默认要求条款确认和邮箱验证；邮箱邀请已证明收件地址，受邀账号可以先绑定并确认契约，之后仍需验证邮箱才能进入业务；账号恢复不能把只有微信身份的账号变成密码账号。
- `/admin` 首次启动自动创建 `admin` / `admin123456`；后台账号可在登录后修改，发布前应完成修改。
...
- `.env`、`.local/`、数据库、备份、日志与真实凭据不得提交；生产模板中的公开默认数据库密码只用于显式初始化，正式部署应改成仅运营方知道的随机值。
